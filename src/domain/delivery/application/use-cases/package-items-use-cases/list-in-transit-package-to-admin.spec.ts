...
import { ListInTransitPackageItemToAdminUseCase } from './list-in-transit-package-to-admin'
...
let sut: ListInTransitPackageItemToAdminUseCase

describe('List in transit package items to admin tests', () => {
  beforeEach(() => {
    ...
    sut = new ListInTransitPackageItemToAdminUseCase(
  ...
  test('If an admin can list all in transit package items', async () => {
  ...
  test('If a courier that is not admin cannot list all in transit items', async () => {
    const courier1 = makeCourier()
  ...
  test(' If unknown user cannot list all in transit items', async () => {
  ...
  test('If an inactive admin cannot list all in transit packages', async () => {
  ...
  test('If system does not have any in transit package items return empty array', async () => {
    const adminId = 'Valid Admin'
    mockAuthorizationService.addActiveAdminId(adminId)
    const packageItem1 = makePackageItem({
      title: 'package1',
