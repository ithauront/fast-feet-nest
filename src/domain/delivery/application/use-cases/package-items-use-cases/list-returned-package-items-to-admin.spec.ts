...
import { ListReturnedPackageItemToAdminUseCase } from './list-returned-package-items-to-admin'
...
let sut: ListReturnedPackageItemToAdminUseCase

describe('List returned package items to admin tests', () => {
  beforeEach(() => {
    ...
    sut = new ListReturnedPackageItemToAdminUseCase(
  ...
  test('If an admin can list all returned package items', async () => {
  ...
  test('If a courier that is not admin cannot list all returned items', async () => {
    const courier1 = makeCourier()
  ...
  test(' If unknown user cannot list all returned items', async () => {
  ...
  test('If an inactive admin cannot list all returned packages', async () => {
  ...
  test('If system does not have any returned package items return empty array', async () => {
    const adminId = 'Valid Admin'
    mockAuthorizationService.addActiveAdminId(adminId)
    const packageItem1 = makePackageItem({
      title: 'package1',
