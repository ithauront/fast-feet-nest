...
import { ListLostPackageItemToAdminUseCase } from './list-lost-package-items-to-admin'
...
let sut: ListLostPackageItemToAdminUseCase

describe('List lost package items to admin tests', () => {
  beforeEach(() => {
    ...
    sut = new ListLostPackageItemToAdminUseCase(
  ...
  test('If an admin can list all lost package items', async () => {
  ...
  test('If a courier that is not admin cannot list all lost items', async () => {
    const courier1 = makeCourier()
  ...
  test(' If unknown user cannot list all lost items', async () => {
  ...
  test('If an inactive admin cannot list all lost packages', async () => {
  ...
  test('If system does not have any lost package items return empty array', async () => {
    const adminId = 'Valid Admin'
    mockAuthorizationService.addActiveAdminId(adminId)
    const packageItem1 = makePackageItem({
      title: 'package1',
