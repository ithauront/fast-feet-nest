...
export abstract class WatchedList<T extends Immutable> {
  ...
  public remove(item: T): void {
    if (this.isImmutableItem(item)) {
      throw new Error('Cannot remove an immutable item.')
    }
  ...
  public update(items: T[]): void {
    ...
    const removedItems = this.getItems().filter((a) => {
      return !items.some((b) => this.compareItems(a, b)) && !a.isImmutable
    })

    this.currentItems = items.concat(
      this.getItems().filter((item) => item.isImmutable),
    )
    this.new = newItems
    this.removed = removedItems
  }

  private isImmutableItem(item: T): boolean {
