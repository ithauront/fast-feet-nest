...
@Controller('/package_item/:courierId/list/delivered')
export class ListDeliveredCourierPackageItemController {
  constructor(
    private listCourierDeliveredPackageItem: ListCourierPackageItemDeliveredUseCase,
  ) {}
  ...
  async handle(
    ...
    const result = await this.listCourierDeliveredPackageItem.execute({
      creatorId: user.sub,
      page,
      courierId,
    })
